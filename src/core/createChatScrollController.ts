...
export function createChatScrollController<TMessage>(
  ...
  function runCheck(): void {
    ...
    const topThreshold = normalizeThreshold(options.thresholdPx, DEFAULT_THRESHOLD_PX);
  ...
  async function loadBottom(): Promise<void> {
    if (!options.onLoadBottom || loadingBottom || destroyed || options.disabled) {
      return;
    }
...
function assertContainer(container: HTMLElement | null | undefined): asserts container is HTMLElement {
  if (
    !container ||
